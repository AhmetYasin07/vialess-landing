...
import { Calendar, User, ArrowRight, Mail, Globe } from 'lucide-react';
...
export default function BlogPage({ onNavigateToPost }: BlogPageProps) {
  ...
  // Category display translation map (internal IDs stay Turkish for filtering)
  ...
  const langFilters: { key: LangFilter; label: string; flag: string }[] = [
    { key: 'all', label: t.blog_lang_all, flag: '🌐' },
    { key: 'tr', label: t.blog_lang_tr, flag: '🇹🇷' },
    { key: 'en', label: t.blog_lang_en, flag: '🇬🇧' },
  ];

  const filteredPosts = blogPosts.filter(post => {
    const categoryMatch = selectedCategory === 'Tümü' || post.category === selectedCategory;
    const langMatch = selectedLang === 'all' || post.lang === selectedLang;
    return categoryMatch && langMatch;
  });

  const handleSubscribe = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`${email} ${t.blog_subscribe_success}`);
    setEmail('');
  };
  ...
  const getLangBadge = (lang: 'tr' | 'en') => {
    if (lang === 'tr') {
      return (
        <span className="inline-flex items-center gap-1 bg-red-50 text-red-700 border border-red-200/60 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase">
          <span>🇹🇷</span>
          <span>TR</span>
        </span>
      );
    ...
    return (
      <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-200/60 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase">
        <span>🇬🇧</span>
        <span>EN</span>
      </span>
    );
          ...
          <div className="grid lg:grid-cols-3 gap-12">
              ...
              <div className="mb-8 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 text-xs text-gray-400 font-medium uppercase tracking-wider">
                    ...
                    <span>{t.blog_lang_filter_title}</span>
                  </div>
              ...
              {filteredPosts.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-400 text-lg">
                    {selectedLang === 'en' ? 'No posts found in English for this category.' : 
                     selectedLang === 'tr' ? 'Bu kategoride Türkçe yazı bulunamadı.' :
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 relative overflow-hidden">
                ...
                <h4 className="text-gray-900 mb-3 font-bold text-lg">{t.blog_newsletter_title}</h4>
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
                <h4 className="text-gray-900 mb-6 font-bold text-lg flex items-center gap-2">
                  ...
                  {t.blog_lang_filter_title}
                </h4>
                <div className="space-y-2">
                  {langFilters.map((filter) => {
                    const count = filter.key === 'all' 
                      ? blogPosts.length 
                      : blogPosts.filter(p => p.lang === filter.key).length;
                    return (
                      <button
                        key={filter.key}
                        onClick={() => setSelectedLang(filter.key)}
                      ...
                      >
                        <span className="flex items-center gap-2">
                          <span className="text-base">{filter.flag}</span>
                          <span>{filter.label}</span>
                        </span>
